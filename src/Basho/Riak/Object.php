...
class Object
{
     ...
     */
    public function __construct(Riak $client, $bucket, $key = null)
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->key = $key;
     ...
     * @throws Exception
     * @internal param int $dw - DW-value, wait for this many partitions to
     * confirm the write before returning to client.
     * @return $this
     */
    public function store($w = null)
    {
        ...
        $url = $this->client->transport->buildRestPath($this->bucket, $this->key, null, $params);
        ...
        # Run the operation.
        $response = $this->client->transport->httpRequest($method, $url, $headers, $content);
    ...
    public function delete($dw = null)
    {
        ...
        # Run the operation...
        $response = $this->client->transport->httpRequest('DELETE', $url);
    ...
    /**
     * Populate object with Transport::httpRequest expected statuses
     *
     * Given the output of Transport::httpRequest and a list of
    ...
    public function populate($response, $expected_statuses)
    {
        ...
        if ($status == 0) {
            $m = 'Could not contact Riak Server: ' . $this->client->schema . '://' . $this->client->host . ':' . $this->client->port . '!';
